...
def fillInExcel(userDetails, rowNum):
    ...
    for i in range(1, sourceWs.max_column+1):
        ...
        match sourceWs[get_column_letter(i) + "1"].value.lower():
            ...
            case "bundesland":
                newWs[get_column_letter(i) + rowNum] = Utility.plzZuBundesland(userDetails[1]["plz"])
            case "telefon":
            ...
            case "telefonnummer":
                newWs[get_column_letter(i) + rowNum] = userDetails[1]["telefon1"] if userDetails[1]["telefon1"] != "" else userDetails[1]["telefon2"] if userDetails[1]["telefon2"] != "" else userDetails[1]["telefon3"]
            case "lv":
                newWs[get_column_letter(i) + rowNum] = Utility.stammesIdToLV(userDetails[1]["gruppierung"])[1]
            case "geburtstag":
                newWs[get_column_letter(i) + rowNum] = Utility.formatDate(str(userDetails[1]["geburtsDatum"])[0:10])
            case "geburtsdatum":
                newWs[get_column_letter(i) + rowNum] = Utility.formatDate(str(userDetails[1]["geburtsDatum"])[0:10])
            case "email":
            ...
            case "e/h":
                newWs[get_column_letter(i) + rowNum] = "E" if UID.userTätigkeit(nami, userDetails[0], "hauptamtlicheIDs.csv") == "ERROR: keine Tätigkeit (ERROR)" else "H"
            case "tätigkeit/bula":
                newWs[get_column_letter(i) + rowNum] = UID.userTätigkeit(nami, userDetails[0], "bulaTätigkeitenIDs.csv").split("(")[0]
            case "tätigkeit/bund":
                newWs[get_column_letter(i) + rowNum] = UID.userTätigkeit(nami, userDetails[0], "bundesTätigkeitIDs.csv").split("(")[0]
            case "tätigkeit/bund-365":
                newWs[get_column_letter(i) + rowNum] = UID.userTätigkeit(nami, userDetails[0], "bundesTätigkeitIDs.csv", -365).split("(")[0]
            case "tätigkeit/hauptamtlich":
                newWs[get_column_letter(i) + rowNum] = UID.userTätigkeit(nami, userDetails[0], "hauptamtlicheIDs.csv").split("(")[0]
